...
ftp::Config::Config() : ip(""), port(0), username(""), password(""), connection_mode(0), is_sftp(false) {}

ftp::Config::Config(const std::string& ip, uint16_t port, const std::string& username, const std::string& password, int32_t mode, bool is_sftp)
    : ip(ip), port(port), username(username), password(password), connection_mode(mode), is_sftp(is_sftp) {
    if (is_sftp) {
        root_url = format("sftp://%s:%d/", ip.c_str(), port);
    } else {
...
bool ftp::upload_as(ftp::Config* config, const char* local_path, const char* upload_file_as, const char* rename_file_to, int& curl_code) {
    ...
    if (nullptr == config || nullptr == local_path || nullptr == upload_file_as || nullptr == rename_file_to) {
        curl_code = -1;
        return false;
    }
...
bool ftp::ssl::upload(Config* config, const char* src, const char* dst, int& curl_code) {
    CURLcode res;
    curl_code = -1;
    struct stat file_info;
    ...
    std::string path = format("sftp://%s:%s@%s/%s", config->username.c_str(), config->password.c_str(), config->ip.c_str(), dst);

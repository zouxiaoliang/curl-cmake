...
bool easy::utils::httpcli::download(const char *url, const char *local_name, int &curl_code, time_t timeout, bool is_continue)
{
    off_t seek = 0;
    CURL* curl = curl_easy_init();
    if (NULL == curl)
...
bool easy::utils::httpcli::request(const char *url, std::string &response, int &curl_code)
{
    CURL* curl = curl_easy_init();
    if (NULL == curl)
    {
    ...
    curl_easy_cleanup(curl);
    curl_code = res;
    return CURLE_OK == res && 200 == response_code;
}

bool easy::utils::httpcli::request(const char *url, const std::map<std::string, std::string> &args, std::string &response, int &curl_code)
{
    ...
    return request(request_url.c_str(), response, curl_code);
}
